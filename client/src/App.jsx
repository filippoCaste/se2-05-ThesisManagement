...
function App() {
  ...
  useEffect(() => {
    userAPI
      .getUserInfo()
      .then((userInfo) => {
        if (userInfo?.email[0] === 's') setUser(new Student(userInfo));
        else if (userInfo?.email[0] === 'd') setUser(new Professor(userInfo));
      })
      .catch((err) => {
        console.log(err);
      });
      ...
      <BrowserRouter>
        <UserContext.Provider value={{ user, setUser }}>
          ...
          <Routes>
            ...
            />
            <Route
              path="/teacher/browseApplications"
              element={<BrowseApplicationsComponent />}
            />
          </Routes>
